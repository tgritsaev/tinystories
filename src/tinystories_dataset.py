...
class TinyStoriesDataset(Dataset):
    def __init__(self, part: str, data_path: str, limit: int = None):
        ...
        self.idxs = torch.from_numpy(np.load(f"{data_path}/{part}_idxs.npy")).astype(torch.int64)[:limit]
        self.data = torch.from_numpy(np.load(f"{data_path}/{part}.npy")).astype(torch.int16)
